function solution(expression) {
  ...
  for (let i = 0; i < operator.length; i++) { // 정해놓은 우선순위 경우의 수 6가지만큼 반복
    const temp = expression.split(/([\+\-\*])/); // 수식 쪼개서 temp 저장
    for (let j = 0; j < temp.length; j++)
      if (temp[j].match(/[0-9]/)) // 문자를 숫자로
        temp[j] = Number(temp[j]);

    for (let j = 0; j < operator[i].length; j++) { // 수식 3개만큼 반복
      while (temp.includes(operator[i][j])) { // 정해놓은 수식의 우선순위 순서대로 수식에 포함된 모든 연산자를 수행.
        const index = temp.indexOf(operator[i][j]); // 일치하는 수식의 인덱스
        if (operator[i][j] === "+") // 덧셈
          temp[index - 1] += temp[index + 1];
        else if (operator[i][j] === "-") // 뺄셈
          temp[index - 1] -= temp[index + 1];
        else if (operator[i][j] === "*") // 곱셈
          temp[index - 1] *= temp[index + 1];
