function solution(m, musicinfos) {
    ...
    for (let i = 0; i < musicinfos.length; i++) {
        ...
        startTime =
            Number(musicInfo[0].split(':')[0]) * 60 +
            Number(musicInfo[0].split(':')[1]);
        endTime =
            Number(musicInfo[1].split(':')[0]) * 60 +
            Number(musicInfo[1].split(':')[1]);
        ...
        if (playTime[i] <= musicInfo[3].length) {
            ...
            for (
                let j = 0;
                j < parseInt(playTime[i] / tempLylics[i].length) - 1;
                j++
            ) {
                musicInfo[3] += tempLylics[i];
            }
            if (playTime[i] % tempLylics[i].length !== 0) {
                musicInfo[3] += tempLylics[i].substr(
                    0,
                    playTime[i] % tempLylics[i].length
                );
            }
        }
...
console.log(
    solution('ABC', ['13:00,13:05,WORLD,ABCDEF', '12:00,12:14,HELLO,C#DEFGAB'])
);
